#!/usr/bin/env python3
"""
Generate threshold diagnostics and prediction overlays for a Faster R-CNN checkpoint.

Example:
  python scripts/diagnose_fasterrcnn_predictions.py \
    --data-dir data/generated/video_frame_smoke_v4 \
    --split-json data/splits/video_frame_smoke_v4/split.json \
    --checkpoint-dir outputs/fasterrcnn_video_frame_smoke_v4_stable/best \
    --out-dir outputs/fasterrcnn_video_frame_smoke_v4_stable/diagnostics \
...
def main() -> None:
    ...
    model = build_model_from_checkpoint(args.checkpoint_dir).to(args.device)
    model.eval()
        ...
        for group_name, group_rows in [("top_positive", top_positive), ("top_negative", top_negative)]:
            for row in group_rows:
                ...
                for score, label, box in zip(outputs["scores"], outputs["labels"], outputs["boxes"]):
                    ...
                    if score_v < args.overlay_threshold:
                        continue
                    x1, y1, x2, y2 = box.tolist()
                    preds.append(
                        {
